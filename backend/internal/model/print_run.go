...
type PrintRun struct {
	...
	// PlannedCopies is 计划份数 recorded when the batch is opened. Partial
	// releases may never cumulatively exceed it.
	PlannedCopies int `json:"plannedCopies" gorm:"not null;default:0"`
	// ReleasedCopies and RemainingCopies are derived from RunRelease rows and
	// are never persisted, so the ledger stays the single source of truth.
	ReleasedCopies  int                `json:"releasedCopies" gorm:"-"`
	RemainingCopies int                `json:"remainingCopies" gorm:"-"`
