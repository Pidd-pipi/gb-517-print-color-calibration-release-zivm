...
func handleError(c *gin.Context, err error) {
	...
	case errors.Is(err, repository.ErrReleaseConflict):
		util.Fail(c, http.StatusConflict, "release_conflict", err.Error())
	case errors.Is(err, repository.ErrReleaseOverflow), errors.Is(err, repository.ErrReleaseState):
		util.Fail(c, http.StatusUnprocessableEntity, "release_rejected", err.Error())
